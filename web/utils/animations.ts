...
export interface ActionDef {
  ...
  frames: number // 一輪幾幀
  loop: boolean
  // 提示文字不放這裡：模擬器下方那行「給美術的規格說明」統一走 i18n 的 actionNote.<id>
  // （composables/useI18n.ts），中英各一份，避免兩處各寫一份而走鐘。
...
export const ACTIONS: ActionDef[] = [
  { id: 'egg', label: '破殼 Egg', group: '生命週期', frames: ANIM_FRAMES + 1, loop: false, sprites: ['egg'], render: renderEgg },
  { id: 'idle', label: '待機 Idle', group: '日常', frames: 40, loop: true, deviceFrameMs: 70, sprites: ['idle', 'bg_room'], render: renderIdle },
  { id: 'yawn', label: '打哈欠 Yawn', group: '日常', frames: 48, loop: true, deviceFrameMs: 70, sprites: ['yawn', 'idle', 'bg_room'], render: renderYawn },
  { id: 'cheer', label: '加油 Cheer', group: '日常', frames: 90, loop: true, deviceFrameMs: 70, sprites: ['cheer', 'idle', 'bg_room'], render: renderCheer },
  { id: 'eat', label: '餵食 Eat', group: '互動', frames: EAT_FRAMES, loop: false, deviceFrameMs: 70, sprites: ['eat', 'bg_room', 'food_0'], picker: 'food', render: renderEat },
  { id: 'sleep', label: '睡覺 Sleep', group: '日常', frames: 40, loop: true, sprites: ['sleep'], render: renderSleep },
  { id: 'pet', label: '摸頭 Pet', group: '互動', frames: 160, loop: true, deviceFrameMs: 70, sprites: ['pet', 'idle', 'bg_room'], render: renderPet },
  { id: 'ending', label: '結局 Ending', group: '結局', frames: 40, loop: true, sprites: ['end_good', 'end_normal', 'end_bad', 'end_runaway'], picker: 'ending', render: renderEnding },
  { id: 'result', label: '摸頭結算 Result', group: '結局', frames: 40, loop: true, sprites: ['emo_success', 'emo_fail'], picker: 'grade', render: renderResult },
]
