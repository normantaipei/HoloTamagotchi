  ...
  <div class="editor">
    <div class="ed-head">
      <div class="ed-title">
        ...
        <span class="desc">{{ t('desc.' + partKey) }}</span>
      </div>
      <div class="ed-meta">{{ t('ed.suggested', { w: spec.w, h: spec.h, path: spec.folder + '/' + spec.suggested, n: frames.length, size: formatBytes(MAX_FILE_BYTES) }) }}</div>
    </div>

    <div v-if="rejected.length" class="reject" role="alert">
      <button class="reject-x" :title="t('ed.dismiss')" @click="rejected = []">✕</button>
      <div class="reject-head">{{ t('ed.skippedHead', { total: formatBytes(MAX_TOTAL_BYTES) }) }}</div>
    ...
    <div class="strip">
      ...
      <div v-for="(f, i) in frames" :key="i" class="card">
        ...
        <label class="pic" :title="t('ed.clickReplace', { name: f.name })">
          <img v-if="f.img" :src="f.dataUrl" :alt="f.name" />
          <span v-else class="loading">{{ t('ed.loading') }}</span>
          <input type="file" accept="image/*" @change="onReplace(i, $event)" />
          <span class="replace-hint">{{ t('ed.replace') }}</span>
        </label>
        <!-- 手稿的三個小框：往前移 / 刪除 / 往後移 -->
        <div class="ctrls">
          <button class="ctrl" :disabled="i === 0" :title="t('ed.moveLeft')" @click="store.moveFrame(partKey, i, i - 1)">◀</button>
          <button class="ctrl del" :title="t('ed.deleteFrame')" @click="store.removeFrame(partKey, i)">✕</button>
          <button class="ctrl" :disabled="i === frames.length - 1" :title="t('ed.moveRight')" @click="store.moveFrame(partKey, i, i + 1)">▶</button>
        </div>
      ...
      <label class="card add" :class="{ empty: frames.length === 0 }">
        <div class="plus">＋</div>
        <div class="add-label">{{ frames.length === 0 ? t('ed.uploadFirst') : t('ed.addFrame') }}</div>
        <div class="add-sub">{{ t('ed.multiSelect') }}</div>
        <input type="file" accept="image/*" multiple @change="onAdd($event)" />
      </label>
    </div>

    <p v-if="frames.length === 0" class="tip">
      {{ t('ed.tipEmpty', { label: t('sprite.' + partKey) }) }}
    </p>
    <p v-else-if="frames.length === 1" class="tip">
      {{ t('ed.tipSingle') }}
    </p>
    <p v-else class="tip">
      {{ t('ed.tipMulti', { n: frames.length }) }}
    </p>
  </div>
</template>
